...
static _u32 phyOctetTime[4] = {
    HARDWARE_OCTET_TIME_1M,//8us per octet
    HARDWARE_OCTET_TIME_2M,//4us per octet
    HARDWARE_OCTET_TIME_CODED_S2,//16us per octet
    HARDWARE_OCTET_TIME_CODED_S8,//64 us per octet
};
...
static _u32  phy_hw_get_rx_header_timestamp(void)
{
    return hal_rf_get_rx_header_timestamp(phyCtrl->phy);
}

static _u32  phy_hw_get_packet_time_from_trigger_to_end(_u8 len,_u8 enc)
{
...
static _u32 phy_hw_get_trigger_time_from_header_timestamp(_u32 ts)
{
    if(phyCtrl->dir == PHY_DIR_TX)
    {
...
void phy_obj_init(phy_obj_t* phy)
{
    ...
    phy->hw_get_trigger_time_from_header_ts = phy_hw_get_trigger_time_from_header_timestamp;
