...
emi_config_param_t emi_config_param =
    {
        .emi_tx_payload_len = EMI_TX_PKT_PAYLOAD,
        .emi_access_code    = EMI_ACCESS_CODE,
};
...
void rf_emi_rx_setup(rf_mode_e mode, signed char rf_chn)
{
    ...
    if (mode != RF_MODE_ZIGBEE_250K) {
        rf_access_code_comm(emi_config_param.emi_access_code);     //access code
    }
    rf_set_tx_rx_off();
    rf_set_rxmode();
    delay_us(150);
...
{
    unsigned char rf_data_len   = emi_config_param.emi_tx_payload_len + 1;
    ...
    {
        rf_data_len          = emi_config_param.emi_tx_payload_len + 2;
        rf_tx_dma_len        = rf_tx_packet_dma_len(rf_data_len);
        emi_ble_tx_packet[5] = emi_config_param.emi_tx_payload_len;
        ...
        if (pkt_type == 0) {
            rf_phy_test_prbs9(&emi_ble_tx_packet[6], emi_config_param.emi_tx_payload_len);
        }
        ...
        rf_data_len             = emi_config_param.emi_tx_payload_len + 1;
        rf_tx_dma_len           = rf_tx_packet_dma_len(rf_data_len);
        emi_zigbee_tx_packet[4] = emi_config_param.emi_tx_payload_len + 2;
        ...
        if (pkt_type == 0) {
            rf_phy_test_prbs9(&emi_zigbee_tx_packet[5], emi_config_param.emi_tx_payload_len);
        }
    } else if ((rf_mode == RF_MODE_PRIVATE_1M) || (rf_mode == RF_MODE_PRIVATE_2M)) {
        rf_data_len               = emi_config_param.emi_tx_payload_len + 1;
        Private_TPLL_tx_packet[4] = emi_config_param.emi_tx_payload_len;
        rf_tx_dma_len             = rf_tx_packet_dma_len(rf_data_len);
        ...
        if (pkt_type == 0) {
            rf_phy_test_prbs9(&Private_TPLL_tx_packet[5], emi_config_param.emi_tx_payload_len);
        }
    }
}
...
void rf_emi_tx_burst_setup(rf_mode_e rf_mode, rf_power_level_e power_level, signed char rf_chn, unsigned char pkt_type)
{
    ...
    case RF_MODE_BLE_2M_NO_PN:
        ...
        for (i = 0; i < emi_config_param.emi_tx_payload_len; i++) {
