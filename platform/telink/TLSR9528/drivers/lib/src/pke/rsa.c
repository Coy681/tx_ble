		...
		unsigned int *out,  unsigned int nBitLen)
{
	...
	ret = pke_mod(a, nWordLen, q, (PKE_A(3,tmp_step)), (PKE_B(4,tmp_step)), pWordLen, m2);
	if(PKE_SUCCESS != ret)
	{
		return ret;
	}
	...
	ret = pke_mod(a, nWordLen, p, (PKE_A(3,tmp_step)), (PKE_B(4,tmp_step)), pWordLen, m1);
	if(PKE_SUCCESS != ret)
	{
		return ret;
	}
	...
	//A1 = hq
	ret = pke_mul(m1, q, (PKE_A(1,tmp_step)), pWordLen);
	if(PKE_SUCCESS != ret)
	{
		return ret;
	}
	...
	//out = m2+hq
	uint32_copy((PKE_B(1,tmp_step)), m2, pWordLen);
	uint32_clear((PKE_B(1,tmp_step))+pWordLen, nWordLen-pWordLen);
	return pke_add((PKE_A(1,tmp_step)), (PKE_B(1,tmp_step)), out, nWordLen);
}
