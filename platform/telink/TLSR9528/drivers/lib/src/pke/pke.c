 ...
 * @return   	none.
 */
void pke_load_operand(volatile unsigned int *baseaddr, volatile const unsigned int *data, unsigned int wordLen)
{
	if(baseaddr != data)
	{
...
void pke_clear_interrupt(void)
{
	if(PKE_RISR & 1)
	{
		...
		/*Note: Unlike the commonly used write 1 to clear 0, this flag bit writes 0 to clear 0.*/
	}
	else
	{;}
}
		...
		unsigned int aWordLen)
{
	...
	pke_load_operand((PKE_B(3,step)), modulus, modWordLen); //B3 modulus
	if((step/4) > modWordLen)
	{
		uint32_clear((PKE_B(3,step))+modWordLen, (step/4)-modWordLen);
	}
	else
	{;}

	pke_load_operand((PKE_B(0,step)), a, aWordLen);         //B0 a
	if((step/4) > aWordLen)
	{
		uint32_clear((PKE_B(0,step))+aWordLen, (step/4)-aWordLen);
	}
	else
	{;}
	...
	{
		pke_read_operand((PKE_A(0,step)), ainv, modWordLen);            //A0 ainv
				   ...
				   unsigned int *out, unsigned int wordLen)
{
	...
	pke_load_operand((PKE_B(3,step)), modulus, wordLen);    //B3 modulus
	pke_load_operand((PKE_A(0,step)), a, wordLen);          //A0 a
	pke_load_operand((PKE_B(0,step)), b, wordLen);          //B0 b

	if((step/4) > wordLen)
	{
		uint32_clear((PKE_B(3,step))+wordLen, (step/4)-wordLen);
		uint32_clear((PKE_A(0,step))+wordLen, (step/4)-wordLen);
		uint32_clear((PKE_B(0,step))+wordLen, (step/4)-wordLen);
	}
	else
	{;}
	...
	{
		pke_read_operand((PKE_A(0, step)), out, wordLen);               //A0 result
...
unsigned int pke_add(const unsigned int *a, const unsigned int *b, unsigned int *out, unsigned int wordLen)//---------------------------
{
	...
	pke_load_operand((PKE_A(0,step)), a, wordLen);          //A1 a
	pke_load_operand((PKE_B(0,step)), b, wordLen);          //B1 b
	...
	{
		pke_read_operand((PKE_A(1,step)), out, wordLen);                //A1 result
...
unsigned int pke_sub(const unsigned int *a, const unsigned int *b, unsigned int *out, unsigned int wordLen)//-------------------------------
{
	...
	pke_load_operand((PKE_A(1,step)), a, wordLen);          //A1 a
	pke_load_operand((PKE_B(1,step)), b, wordLen);          //B1 b

	if((step/4) > wordLen)
	{
		uint32_clear((PKE_A(1,step))+wordLen, (step/4)-wordLen);
		uint32_clear((PKE_B(1,step))+wordLen, (step/4)-wordLen);
	}
	else
	{;}
...
unsigned int pke_mul(const unsigned int *a, const unsigned int *b, volatile unsigned int *out, unsigned int ab_wordLen)//-----------------------------
{
	...
	pke_load_operand((PKE_A(0,step)), a, ab_wordLen);       //A0 a
	pke_load_operand((PKE_B(0,step)), b, ab_wordLen);       //B0 b

	uint32_clear((PKE_A(0,step))+ab_wordLen, (step/4)-ab_wordLen);
	uint32_clear((PKE_B(0,step))+ab_wordLen, (step/4)-ab_wordLen);
	...
	{
		pke_read_operand((PKE_A(1,step)), out, tempLen);                //A1 result
 ...
 */
unsigned int pke_pre_calc_mont(volatile const unsigned int *modulus, unsigned int bitLen, unsigned int *H, unsigned int *n1)
{
	...
	{
		uint32_clear((PKE_B(3,step))+wordLen, (step/4)-wordLen);
		uint32_clear((PKE_A(3,step))+wordLen, (step/4)-wordLen);
	}
	else
	{;}
	...
	if(NULL != H)
	{
		pke_read_operand((PKE_A(3,step)), H, wordLen);                  //A3 H
	}
	else
	{;}
	...
	if(NULL != n1)
	{
		pke_read_operand((PKE_B(4,step)), n1, 1);                       //B4 n1
	}
	else
	{;}
...
void pke_load_pre_calc_mont(volatile const unsigned int *H, volatile const unsigned int *n1, unsigned int wordLen)
{
	...
	pke_load_operand((PKE_A(3,step)), H, wordLen);
	if((step/4) > wordLen)
	{
		uint32_clear((PKE_A(3,step))+wordLen, (step/4)-wordLen);
	}
	...
	pke_load_operand((PKE_B(4,step)), n1, 1);
}
		...
		unsigned int wordLen)
{
	...
	{
		pke_read_operand((PKE_A(0,step)), out, wordLen);                //A0 out
		...
		volatile unsigned int *out, unsigned int mod_wordLen, unsigned int exp_wordLen)
{
	...
	pke_load_operand((PKE_A(1,step)), exponent, exp_wordLen);   //A1 exponent
	if((step/4) > exp_wordLen)
	{
		uint32_clear((PKE_A(1,step))+exp_wordLen, (step/4)-exp_wordLen);
	}
	else
	{;}

	pke_load_operand((PKE_B(3,step)), modulus, mod_wordLen);    //B3 modulus
	pke_load_operand((PKE_B(0,step)), base, mod_wordLen);       //B0 base

	if((step/4) > mod_wordLen)
	{
		uint32_clear((PKE_B(3,step))+mod_wordLen, (step/4)-mod_wordLen);
		uint32_clear((PKE_B(0,step))+mod_wordLen, (step/4)-mod_wordLen);
	}
	else
	{;}
	...
	{
		pke_read_operand((PKE_A(0,step)), out, mod_wordLen);                //A0 result
					  ...
					  volatile unsigned int *Qx, volatile unsigned int *Qy)
{
	...
	pke_load_operand((PKE_B(3,step)), curve->eccp_p, wordLen);          //B3 p
	if((step/4) > wordLen)
	{
		uint32_clear((PKE_B(3,step))+wordLen, (step/4)-wordLen);
	}
	...
	{
		pke_pre_calc_mont((PKE_B(3,step)), curve->eccp_p_bitLen, NULL, NULL);
	}
	else
	{
	...
	pke_load_operand((PKE_B(0,step)), Px, wordLen);                     //B0 Px
	pke_load_operand((PKE_B(1,step)), Py, wordLen);                     //B1 Py
	pke_load_operand((PKE_A(5,step)), curve->eccp_a, wordLen);          //A5 a
	pke_load_operand((PKE_A(4,step)), k, wordLen);                      //A4 k
	pke_load_operand((PKE_B(5,step)), curve->eccp_n, wordLen);          //B5 n

	if((step/4) > wordLen)
	{
		uint32_clear((PKE_B(0,step))+wordLen, (step/4)-wordLen);
		uint32_clear((PKE_B(1,step))+wordLen, (step/4)-wordLen);
		uint32_clear((PKE_A(5,step))+wordLen, (step/4)-wordLen);
		uint32_clear((PKE_A(4,step))+wordLen, (step/4)-wordLen);
		uint32_clear((PKE_B(5,step))+wordLen, (step/4)-wordLen);
	}
	else
	{;}
	...
	pke_read_operand((PKE_A(0,step)), Qx, wordLen);                     //A0 Qx
	if(NULL != Qy)
	{
		pke_read_operand((PKE_A(1,step)), Qy, wordLen);                 //A1 Qy
	}
	else
	{;}
					  ...
					  unsigned int *Qx, unsigned int *Qy)
{
	...
	pke_load_operand((PKE_A(0,step)), P1x, wordLen);                    //A0 P1x
	pke_load_operand((PKE_A(1,step)), P1y, wordLen);                    //A1 P1y
	pke_load_operand((PKE_B(0,step)), P2x, wordLen);                    //B0 P2x
	pke_load_operand((PKE_B(1,step)), P2y, wordLen);                    //B1 P2y
	pke_load_operand((PKE_A(5,step)), curve->eccp_a, wordLen);          //A5 a
...
unsigned int eccp_pointDouble(eccp_curve_t *curve, unsigned int *Px, unsigned int *Py, unsigned int *Qx, unsigned int *Qy)
{
	...
	pke_load_operand((PKE_A(0,step)), Px, wordLen);                     //A0 Px
	pke_load_operand((PKE_A(1,step)), Py, wordLen);                     //A1 Py
	pke_load_operand((PKE_A(5,step)), curve->eccp_a, wordLen);          //A5 a
...
unsigned int eccp_pointVerify(eccp_curve_t *curve, unsigned int *Px, unsigned int *Py)
{
	...
	pke_load_operand((PKE_B(1,step)), Py, wordLen);                     //B1 Py
	pke_load_operand((PKE_A(5,step)), curve->eccp_a, wordLen);          //A5 a
	pke_load_operand((PKE_A(4,step)), curve->eccp_b, wordLen);          //A4 b
...
unsigned int eccp_get_pubkey_from_prikey(eccp_curve_t *curve, unsigned char *priKey, unsigned char *pubKey)
{
	...
	{
		reverse_byte_array((volatile unsigned char *)x, pubKey, pByteLen);
		reverse_byte_array((volatile unsigned char *)y, pubKey+pByteLen, pByteLen);
...
unsigned int x25519_pointMul(mont_curve_t *curve, unsigned int *k, const unsigned int *Pu, unsigned int *Qu)
{
	...
	pke_load_operand(PKE_B(3,step), curve->p, wordLen);              //B3 p
	if((step/4) > wordLen)
	{
		uint32_clear(PKE_B(3,step)+wordLen, (step/4)-wordLen);
	}
	...
	{
		pke_pre_calc_mont(PKE_B(3,step), curve->p_bitLen, NULL, NULL);
	}

	pke_load_operand(PKE_A(0,step), Pu, wordLen);                     //A0 Pu
	pke_load_operand(PKE_B(0,step), curve->a24, wordLen);             //B0 a24
	pke_load_operand(PKE_A(4,step), k, wordLen);                      //A4 k
	...
	pke_read_operand(PKE_A(1,step), Qu, wordLen);                     //A1 Qu
...
unsigned int mod_exp(unsigned int a[8], unsigned int b[8], unsigned int n[8], unsigned int out[8])
{
	...
	//t = A0 = aR mod n
	ret = pke_modmul_internal((PKE_B(3,step)), a, (PKE_A(3,step)), t, Ed25519_WORD_LEN);     //A3: R^2 mod n
	if(PKE_SUCCESS != ret)
	{
		return ret;
	}
	...
	for(; bitLen>=0; bitLen--)
	{
		ret = pke_modmul_internal((PKE_B(3,step)), (PKE_A(0,step)), (PKE_A(0,step)),
								  (PKE_A(0,step)), Ed25519_WORD_LEN);
		if(PKE_SUCCESS != ret)
		{
			return ret;
		}
		...
		{
			ret = pke_modmul_internal((PKE_B(3,step)), (PKE_A(0,step)), t, (PKE_A(0,step)),
									  Ed25519_WORD_LEN);
			if(PKE_SUCCESS != ret)
			{
				return ret;
	...
	//get result
	ret = pke_modmul_internal((PKE_B(3,step)), (PKE_A(0,step)), t, out, Ed25519_WORD_LEN);
	if(PKE_SUCCESS != ret)
	{
		return ret;
	}
		...
		unsigned int *Qx, unsigned int *Qy)
{
	...
	pke_load_operand(PKE_A(1,step), Px, wordLen);                    //A1 Px
	pke_load_operand(PKE_A(2,step), Py, wordLen);                    //A2 Py
	pke_load_operand(PKE_B(0,step), curve->d, wordLen);              //B0 d
	pke_load_operand(PKE_A(0,step), k, wordLen);                     //A0 k

	if((step/4) > wordLen)
	{
		uint32_clear(PKE_A(1,step)+wordLen, (step/4)-wordLen);
		uint32_clear(PKE_A(2,step)+wordLen, (step/4)-wordLen);
		uint32_clear(PKE_B(0,step)+wordLen, (step/4)-wordLen);
		uint32_clear(PKE_A(0,step)+wordLen, (step/4)-wordLen);
	}
	else
	...
	pke_read_operand(PKE_A(1,step), Qx, wordLen);                    //A1 Qx
	if(NULL != Qy)
	{
		pke_read_operand(PKE_A(2,step), Qy, wordLen);                //A2 Qx
	}
	else
	{;}
		...
		unsigned int *Qx, unsigned int *Qy)
{
	...
	pke_load_operand(PKE_A(1,step), P1x, wordLen);                   //A1 P1x
	pke_load_operand(PKE_A(2,step), P1y, wordLen);                   //A2 P1y
	pke_load_operand(PKE_B(1,step), P2x, wordLen);                   //B1 P2x
	pke_load_operand(PKE_B(2,step), P2y, wordLen);                   //B2 P2y
	pke_load_operand(PKE_B(0,step), curve->d, wordLen);              //B0 d
	...
	if((step/4) > wordLen)
	{
		...
		uint32_clear(PKE_A(2,step)+wordLen, (step/4)-wordLen);
		uint32_clear(PKE_B(1,step)+wordLen, (step/4)-wordLen);
		uint32_clear(PKE_B(2,step)+wordLen, (step/4)-wordLen);
		uint32_clear(PKE_B(0,step)+wordLen, (step/4)-wordLen);
	}
	else
	{;}
	...
	pke_read_operand(PKE_A(1,step), Qx, wordLen);                    //A1 Qx
	pke_read_operand(PKE_A(2,step), Qy, wordLen);                    //A2 Qy
