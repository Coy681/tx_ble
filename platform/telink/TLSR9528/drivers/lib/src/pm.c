...
_attribute_ram_code_sec_noinline_ void pm_stimer_recover(void)
{
    ...
    while ((reg_system_st & BIT(7)) == 0)
        ;                    //system timer set done status upon next 32k posedge
...
_attribute_ram_code_sec_noinline_ void pm_update_status_info(unsigned char clr_en) //BLE SDK use: to put into RAM
{
    ...
    if (wd_clr0 & POWERON_FLAG) {
        if (poweron_clr0 & REBOOT_FLAG) {
            ...
            if (clr_en == 1) {
                analog_write_reg8(PM_ANA_REG_WD_CLR_BUF0, wd_clr0 & (~POWERON_FLAG));
                analog_write_reg8(PM_ANA_REG_POWER_ON_CLR_BUF0, (poweron_clr0 & (~SW_REBOOT_REASON)) | REBOOT_FLAG);
            }
        }
    } else {
...
{
    /**
     * Probe voltage to GPIO_PD7.
     */
    analog_write_reg8(0x1b, analog_read_reg8(0x1b) | BIT(3));
    gpio_function_en(GPIO_PD7);
    gpio_output_dis(GPIO_PD7);
    gpio_input_en(GPIO_PD7);
}
