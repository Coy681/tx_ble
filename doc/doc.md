...
从零到一
   ...
   -- 使用system call的方式，系统分层初始化
      link文件新增init section,通过定义static const变量的方式，将函数名存储到一个特殊的init section里面
   -- 简化的事件驱动模型
      任务虚拟成节点（具有init，process，priority等属性），
   -- log模块简单实现
      使用Uart模拟串口log输出，固定格式（字符串+数据），字符串直接copy，数据要转换为字符串
   -- 内存管理模块
      --几种常用的内存管理模型对比
